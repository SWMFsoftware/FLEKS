...
void write_variables(int nvars, int *vardim, int *centering,
                     const char *const *varname, float **vars, int npts,
                     int ncells) {
  char str[1024];
  ...
  new_section();
  snprintf(str, sizeof str, "CELL_DATA %d\n", ncells);
  write_string(str);
  ...
  new_section();
  snprintf(str, sizeof str, "POINT_DATA %d\n", npts);
  write_string(str);
...
void write_rectilinear_mesh(const char *filename, int ub, int *dims, float *x,
                            float *y, float *z, int nvars, int *vardim,
                            int *centering, const char *const *varnames,
                            float **vars) {
  int i;
  char str[128];
  ...
  snprintf(str, sizeof str, "DIMENSIONS %d %d %d\n", dims[0], dims[1], dims[2]);
  write_string(str);
