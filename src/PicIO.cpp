...
void Pic::find_mpi_rank_for_points(const int nPoint, const double* const xyz_I,
  ...
  const RealBox& range = get_gem(0).ProbDomain();
    ...
    for (int iSpecies = 0; iSpecies < nSpecies; iSpecies++)
      for (int iVar = iRho_; iVar <= iPyz_; iVar++) {
        ...
        dataPIC_I[iStart + iVar] =
            get_value_at_loc(nodePlasma[iSpecies], get_gem(0), xp, yp, zp, iVar);
      }

    for (int iDir = ix_; iDir <= iz_; iDir++) {
      dataPIC_I[iBx_ + iDir] = get_value_at_loc(nodeB, get_gem(0), xp, yp, zp, iDir);
    }

    for (int iDir = ix_; iDir <= iz_; iDir++) {
      dataPIC_I[iEx_ + iDir] = get_value_at_loc(nodeE, get_gem(0), xp, yp, zp, iDir);
    }
...
void Pic::find_output_list(const PlotWriter& writerIn, long int& nPointAllProc,
  ...
  const auto plo = get_gem(0).ProbLo();
  const auto plh = get_gem(0).ProbHi();
  ...
  const Box& gbx = convert(get_gem(0).Domain(), { 1, 1, 1 });
  ...
  for (MFIter mfi(nodeE); mfi.isValid(); ++mfi) {
    ...
    for (int iDim = 0; iDim < nDim; iDim++)
      if ((get_gem(0).isPeriodic(iDim)) && gbx.bigEnd(iDim) == hi[iDim])
        hi[iDim]--;
    ...
    auto do_output_this_node = [&](int i, int j, int k) {
      ...
      if (type != iAbandon_) {
        if (get_gem(0).isPeriodic(ix_) && i == glo.x && !test_bit(type, ix_))
          return true;
        if (get_gem(0).isPeriodic(iy_) && j == glo.y && !test_bit(type, iy_))
          return true;
        if (!isFake2D && get_gem(0).isPeriodic(iz_) && k == glo.z &&
            !test_bit(type, iz_))
          return true;
      }
  ...
  if (ParallelDescriptor::MyProc() == 0 && writerIn.get_plotDx() >= 0) {
    ...
    Box gbx = convert(get_gem(0).Domain(), { 1, 1, 1 });
    ...
    if (get_gem(0).isPeriodic(ix_))
      --iMax;
    if (get_gem(0).isPeriodic(iy_))
      --jMax;
    if (get_gem(0).isPeriodic(iz_))
      --kMax;
...
void Pic::set_IO_geom(amrex::Geometry& geomIO, const PlotWriter& pw) {
  ...
  for (int i = 0; i < nDim; i++) {
    boxRangeOut.setLo(i, get_gem(0).ProbLo(i) * no2outL);
    boxRangeOut.setHi(i, get_gem(0).ProbHi(i) * no2outL);
  }
  Array<int, nDim> periodicity;
  for (int i = 0; i < nDim; i++)
    periodicity[i] = get_gem(0).isPeriodic(i);
  geomIO.define(get_gem(0).Domain(), boxRangeOut, get_gem(0).Coord(), periodicity);
}
  ...
  if (plotVars.find("X") != std::string::npos) {
    ...
    for (MFIter mfi(xyz); mfi.isValid(); ++mfi) {
      ...
      for (int k = lo.z; k <= hi.z; ++k) {
        // Use the value returned from get_gem(0) instead of geomOut, and it will be
        // converted to output unit just as other variables.
        const Real z0 = get_gem(0).CellCenter(k, iz_);
        for (int j = lo.y; j <= hi.y; ++j) {
          const Real y0 = get_gem(0).CellCenter(j, iy_);
          for (int i = lo.x; i <= hi.x; ++i) {
            const Real x0 = get_gem(0).CellCenter(i, ix_);
