...
The electric field is assembled on the node grid from the node-centred ion fluid moments and the cell-centred magnetic field:
\begin{equation}
...
The hybrid solver uses the same grid layout as the full-PIC solver: the electric field $\mathbf{E}$, the current density $\mathbf{J}$, and the ion moments $\rho$, $\rho\mathbf{u}$ are stored on the node grid, while the magnetic field $\mathbf{B}$ and the scalar electron pressure $P_e$ are stored at cell centres. The generalized Ohm's law is therefore assembled at the nodes from the node-centred moments and the cell-centred $\mathbf{B}$, and the Faraday update advances the cell-centred $\mathbf{B}$; the node-centred $\mathbf{B}$ that the Boris push samples is recovered by averaging the surrounding cell values. The cell-centred field is differentiated with the compact $2\,\Delta x$ central-difference stencil
\begin{equation}
...
\begin{enumerate}
  ...
  \item \textbf{Faraday advance:} Subcycled explicit RK4 or SSPRK3 advances the cell-centred $\mathbf{B}^n \to \mathbf{B}^{n+1}$ using the interpolated ion moments; the node-centred $\mathbf{B}^{n+1}$ sampled by the pusher is obtained by averaging the surrounding cell values.
  \item \textbf{Electric field assembly:} The node-centred $\mathbf{E}^{n+1}$ is computed from eq.~\eqref{eq:ohmslaw} and stored for the subsequent Boris push.
\end{enumerate}
